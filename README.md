...
[![Version](https://img.shields.io/badge/Version-0.0.74-brightgreen)]()
...
* 📡 **Удобный Web UI в Home Assistant (Ingress)**:
  * Управление «Мозгом» (модель, температура, поиск Google, Barge-in, 4 каноничных промпта).
  * «Флот колонок» — карточки всех устройств со статусом, пингом и персональными настройками.
  * «Групповая настройка» — быстрое применение параметров на все ESP32 сразу.
  * Управление кэшем фраз и дакингом мультимедиа.
* 🔄 **One-Click OTA обновления по WebSocket**: Аддон автоматически раздаёт свежие прошивки на все подключенные колонки без проводов.
