...
### Версия 0.0.13 (Нативная прошивка ESP32-S3)
*   **Отказ от ESPHome**: Переход на голую C++ прошивку PlatformIO для полного контроля над I2S-микрофоном (INMP441, INMP414) и динамиком (MAX98357A).
*   **Локальный Wake Word**: Конвертация `porfiriy.tflite` в C-массив `model.h`. Вейкворд детектируется локально (заглушка готова, ожидает интеграции STFT-фильтров), после чего открывается стриминг по WebSocket к аддону.
*   **Captive Portal**: Настройка Wi-Fi и IP-адреса сервера больше не зашита в код. Если сеть недоступна, ESP32 поднимает открытую точку `Porfiriy_Setup`, откуда раздает HTML-страницу настроек и сохраняет их в `Preferences` (NVS).
